                               ...
                               const Coordinates<data_t> &coords) const
{
    ...
    // 4D levi civita symbol and 3D levi civita tensor
    const std::array<std::array<std::array<std::array<int, 4>, 4>, 4>, 4> epsilon4 =
        TensorAlgebra::epsilon4D();
    Tensor<3, data_t> epsilon3;
    ...
    FOR3(i, j, k)
    {
        for (int l = 0; l < 4; ++l)
        {
            epsilon3[i][j][k] += n_U[l] * epsilon4[i][j][k][l] * vars.lapse * pow(vars.chi, -1.5);
        }
    }
    // rasing indices
    ...
    FOR3(i, j, k)
    {
        FOR2(m, n)
        {
            out.epsilon3_LUU[i][j][k] += epsilon3[i][m][n] * h_UU[m][j] * vars.chi *
                                     h_UU[n][k] * vars.chi;
        }
    }
    ...
    {
        out.B[i][j] +=
            out.epsilon3_LUU[i][k][l] * (covariant_deriv_K_tensor[l][j][k]);
    }
